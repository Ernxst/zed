...
impl Interactivity {
    ...
    fn scroll_max(&self, bounds: Bounds<Pixels>, style: &Style, window: &Window) -> Point<Pixels> {
        ...
        // Taffy lays the box out with padding snapped to the device-pixel grid.
        // Recompute it the same way so fractional rem sizes do not manufacture a
        // sub-pixel scroll range.
        let padding = style
            .padding
    ...
    fn compute_style_internal(
        ...
        if !cx.has_active_drag() {
            if let Some(group_hover) = self.group_hover_style.as_ref() {
                ...
                {
                    !window.last_input_was_touch() && group_hitbox_is_hovered(&group_hitbox, window)
...
mod tests {
    ...
    use crate::{
        ...
        MouseMoveEvent, TestAppContext, TouchEvent, TouchId, VisualTestContext, canvas, svg,
    ...
    #[gpui::test]
    fn test_fractional_padding_preserves_subpixel_overflow(cx: &mut TestAppContext) {
        ...
        impl Render for PaddedContainer {
            fn render(
                ...
                // Authored fractional padding remains fractional through layout. The
                // 50.5px total extent rounds to a 51px scroll size, matching browsers.
                div().size_full().child(
                    div()
                        .id("container")
                        .h(px(50.))
        ...
        assert_eq!(scroll_handle.max_offset().y, px(1.));
    }
