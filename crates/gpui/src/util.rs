...
#[inline]
pub(crate) fn round_to_device_pixel_f64(logical: f64, scale_factor: f64) -> f64 {
    (logical * scale_factor + 0.5).floor()
}
...
mod tests {
    ...
    fn test_round_to_device_pixel() {
        ...
        assert_eq!(round_to_device_pixel(-0.5, 1.0), 0.0);
        assert_eq!(round_to_device_pixel(-1.5, 1.0), -1.0);
        assert_eq!(round_to_device_pixel(-2.5, 1.0), -2.0);

        // Non-midpoint values round to nearest
        assert_eq!(round_to_device_pixel(1.5001, 1.0), 2.0);
        assert_eq!(round_to_device_pixel(1.4999, 1.0), 1.0);
        assert_eq!(round_to_device_pixel(-1.5001, 1.0), -2.0);
        assert_eq!(round_to_device_pixel(-1.4999, 1.0), -1.0);

        // Integers are unchanged
        assert_eq!(round_to_device_pixel(0.0, 1.0), 0.0);
        assert_eq!(round_to_device_pixel(3.0, 1.0), 3.0);
        assert_eq!(round_to_device_pixel(-3.0, 1.0), -3.0);
    }

    #[test]
    fn test_device_pixel_helpers() {
        // Snap uses half-up: 1.0 * 1.5 = 1.5 ties toward 2.0.
        assert_eq!(round_to_device_pixel(1.0, 1.5), 2.0);
        ...
        assert_eq!(round_stroke_to_device_pixel(1.0, 1.5), 2.0);
    ...
    #[test]
    fn test_round_to_device_pixel_f64() {
        assert_eq!(round_to_device_pixel_f64(0.5, 1.0), 1.0);
        assert_eq!(round_to_device_pixel_f64(-0.5, 1.0), 0.0);
        assert_eq!(round_to_device_pixel_f64(1.5, 1.0), 2.0);
        assert_eq!(round_to_device_pixel_f64(-1.5, 1.0), -1.0);
        assert_eq!(round_to_device_pixel_f64(2.5001, 1.0), 3.0);
    }
