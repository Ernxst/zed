...
use crate::{
    ...
    WindowBounds, WindowControls, WindowDecorations, WindowInsets, WindowMinSize, WindowOptions,
...
impl Window {
    ...
    pub fn paint_underline_with_exclusions(
        ...
        let current_clip = self.next_frame.scene.clips[underline.clip_id as usize];
        let bounds = underline.bounds.intersect(&current_clip.folded_bounds);
        if bounds.is_empty() {
            return;
        }
        ...
        let mut paint_span = |start, end| {
            let folded_bounds = current_clip.folded_bounds.intersect(&Bounds::from_corners(
    ...
    #[cfg(test)]
    pub(crate) fn clear_scene_for_test(&mut self) {
        let clips = mem::take(&mut self.next_frame.scene.clips);
        self.next_frame.scene.clear();
        self.next_frame.scene.clips = clips;
    }
...
mod tests {
    ...
    use crate::{
        AnyWindowHandle, AppContext as _, Bounds, ClipNode, ContentMask, ContentMaskEntry, Context,
        Corners, DispatchPhase, DragMoveEvent, Empty, ExternalDragPayload, ExternalPaths,
        FileDragPaths, FileDropEvent, FocusHandle, InputEvent as _, InteractiveElement as _,
        IntoElement, KeyDownEvent, Keystroke, LongPressEvent, MouseButton, MouseDownEvent,
    ...
    fn test_underline_exclusions_preserve_device_geometry(cx: &mut TestAppContext) {
        test_underline_paint_at_scales(cx, |window| {
            ...
            let snap = |quarters: i32| {
                ...
                ScaledPixels((2 * scaled + divisor).div_euclid(2 * divisor) as f32)
            };
            let stroke = |quarters| {
                if quarters == 0 {
                    ScaledPixels(0.)
                    ...
                    for thickness in [0, 1, 4, 7] {
                        for wavy in [false, true] {
                            ...
                            for (actual, original) in underlines.iter().zip(&original) {
                                ...
                                assert_eq!(actual.clip_id, original.clip_id);
                            }
                            ...
                            let expected = (left..right)
                                ...
                                .collect::<Vec<_>>();
                            assert_eq!(underline_device_columns(window), expected);
    ...
    fn test_underline_exclusions_preserve_helvetica_warning_edge(cx: &mut TestAppContext) {
        test_underline_paint_at_scales(cx, |window| {
            ...
            assert_eq!(
                underline_device_columns(window),
                (28..48).collect::<Vec<_>>()
            );
        });
    }
                    ...
                    window.with_element_opacity(Some(0.5), |window| {
                        ...
                        assert_eq!(
                            underlines
                                .iter()
                                .map(|underline| {
                                    window.next_frame.scene.clips[underline.clip_id as usize]
                                        .folded_bounds
                                })
                                .collect::<Vec<_>>(),
                            expected
                ...
                for first_end in endpoints {
                    for second_start in endpoints {
                        for second_end in endpoints {
                            ...
                            let _underlines = paint_test_underlines(window, |window| {
                                window.paint_underline_with_exclusions(
                                    origin,
                                    width,
                                    &style,
        ...
        fn render(&mut self, _: &mut Window, _: &mut Context<Self>) -> impl IntoElement {
            ...
            canvas(
                ...
                move |_, _, window, _| {
                    ...
                    let clip_id = window.next_frame.scene.insert_clip(ClipNode {
                        folded_bounds: window.cover_bounds(mask.bounds),
                        rounded_bounds: Bounds::default(),
                        corner_radii: Corners::default(),
                        rounded_head: ClipNode::NONE,
                        parent_rounded: ClipNode::NONE,
                    });
                    window.content_mask_stack.push(ContentMaskEntry {
                        mask,
                        rounded: None,
                        clip_id,
                        rounded_head: ClipNode::NONE,
    ...
    fn paint_test_underlines(
        ...
        window.clear_scene_for_test();
        paint(window);
        window.next_frame.scene.underlines.clone()
    }
    ...
    fn underline_device_columns(window: &Window) -> Vec<i32> {
        window
            .next_frame
            .scene
            .underlines
            .iter()
            .flat_map(|underline| {
                let bounds = underline.bounds.intersect(
                    &window.next_frame.scene.clips[underline.clip_id as usize].folded_bounds,
                );
